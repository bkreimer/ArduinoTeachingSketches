...
void loop() {
  ...
  // if the brightness value passes 0 or 255 (our max range) reverse the direction
  if (brightness <= 0 || brightness >= 255) {
    amount *= -1;
  }
  
  // write brightness to the LED with 'analogWrite' using PWM; then wait so we can see the results
