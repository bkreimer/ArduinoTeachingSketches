...
SERIAL COMMUNICATION
 ...
 In order to see the results, you must open the Serial Monitor in the Arduino IDE, located above. ^^^
...
void loop() {
  ...
  if (buttonState == HIGH) {
    ...
    // command, rather than 'println' - the downfall is that in C (the basis for Arduino) we can't
    // simply combine these into one command like in Java, Python, etc
